...
RUN git clone -b docker-img-fix https://github.com/enterprisemediawiki/meza /opt/meza
# COPY . /opt/meza
...
RUN ansible-vault decrypt /opt/conf-meza/secret/monolith/secret.yml --vault-password-file /opt/conf-meza/users/meza-ansible/.vault-pass-monolith.txt \
	...
	&& ansible-vault encrypt /opt/conf-meza/secret/monolith/secret.yml --vault-password-file /opt/conf-meza/users/meza-ansible/.vault-pass-monolith.txt
